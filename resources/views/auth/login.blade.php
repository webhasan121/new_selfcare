<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}" class="auth-form flex flex-col gap-[20px]">
        ...
        <button type="submit" class="auth-submit w-full flex items-center justify-center gap-[16px] p-[15px_17px] rounded-[8px]
         font-semibold text-[12px] text-[#fff] bg-[#108c78] shadow-[0_5px_12px_#108c7815] min-h-[49px] [&:hover]:bg-[#097762] 
         [&:hover]:shadow-[0_6px_16px_#108c7830] [&:hover]:[transform:translateY(-1px)] [&_.care-icon]:w-[17px]
          [&_.care-icon]:h-[17px]">Log In<x-care-icon name="arrow" /></button>
    </form>
        ...
        @csrf
            ...
            <x-auth-field name="user_id" label="User ID" type="text" :value="old('user_id')"
                placeholder="Enter your user ID" :autofocus="true" />
            <x-auth-field name="secret" label="Secret" type="password" autocomplete="current-password"
                placeholder="Enter your secret" />
 
        <div class="auth-form-options flex items-center justify-between gap-[12px] text-[11px] m-[0_0_2px] [&>a]:text-[#118773] [&>a]:font-[650] dark:[&>a]:text-[#73d8b9]"><label class="auth-remember flex items-center gap-[8px] text-[#7b8d85] text-[11px] cursor-pointer [&_input]:w-[15px] [&_input]:h-[15px] [&_input]:[border:1px_solid_#cddbd3] [&_input]:rounded-[4px] [&_input]:text-[#138d79] [&_input:focus]:[--tw-ring-color:#13967d] dark:text-[#a1b8ae] dark:[&_input]:bg-[#20372f] dark:[&_input]:border-[#58796b] dark:[&_input:checked]:bg-[#108c78]"><input id="remember_me" name="remember"
                    type="checkbox" @checked(old('remember'))> Remember me</label><!-- <a
                href="{{ route('password.request') }}">Forgot password?</a> --></div>
        <button type="submit" class="auth-submit w-full flex items-center justify-center gap-[16px] 
        p-[15px_17px] rounded-[8px] font-semibold text-[12px] text-[#fff] bg-[#108c78]
         shadow-[0_5px_12px_#108c7815] min-h-[49px] [&:hover]:bg-[#097762] [&:hover]:shadow-[0_6px_16px_#108c7830]
          [&:hover]:[transform:translateY(-1px)] [&_.care-icon]:w-[17px] [&_.care-icon]:h-[17px]">Log In <x-care-icon name="arrow" /></button>
    </form>
